...
window.AtaviImport = {
	...
	importFinish: function(status) {
		if (status) {
			...
			document.location='https://atavi.com/?from=ext-chrome';
			$('#js-goto-atavi').show();
	...
	bookmarkSend: function(items, callback) {
		var request = 'https://' + AtaviImport.host + '/extension/import?' + '_t=' + Math.random();
	...
	onGotoAtavi: function() {
		document.location = 'https://' + AtaviImport.host + '/?from=ext-chrome';
	},
