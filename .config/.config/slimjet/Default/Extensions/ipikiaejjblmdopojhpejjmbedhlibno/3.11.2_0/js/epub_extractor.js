...
function bfsKeepNodesAtLevel(body, maxLevel = 1) {

    const queue = [];
    let currentEl = body;
    let currentElLevel = 0;
    ...
    while (currentEl) {
        ...
        // console.log('currentEl: ', currentEl); // Do something to element here
        // console.log('currentElLevel: ', currentElLevel);
        // console.log(currentEl.getBoundingClientRect());

        for (let i = 0; i < currentEl.children.length; i++) {
            queue.push([currentEl.children[i], currentElLevel + 1]);
        }

        currentElTuple = queue.shift();
        if (currentElTuple) {
            currentEl = currentElTuple[0];
            currentElLevel = currentElTuple[1];

            if (currentElLevel === maxLevel) {
                nodesToKeep.push(currentEl);
            }

        } else {
            currentEl = undefined;
            currentElLevel = undefined;
        }
...
function extractVisibleNodesFromIframe(iframe, body) {
    ...
    const nodesToKeep = bfsKeepNodesAtLevel(body, 1);
    ...
    for (let i = 0; i < visibleNodes.length; i++) {
        ...
        // iterate through sr-tokens in this node, check if visible
...
function extractText(fromSwiftRead = false) {
    ...
    if (iframes.length > 0) {
        ...
        if (innerText.length === 0 && fromSwiftRead === false) {
            ...
            let visibleHTML = "";
            for (let i = 0; i < visibleNodes.length; i++) {
                visibleHTML += visibleNodes[i].innerHTML;
            }
